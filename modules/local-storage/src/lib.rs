...
pub fn handle_rpc(input: Vec<u8>) -> FnResult<Vec<u8>> {
    let _config: LocalConfig = match config::get("config") {
        ...
        _ => return Err(anyhow::anyhow!("Configuration not found for local-storage").into()),
    };
    ...
    let (payload, error) = match request.method.as_str() {
        "get_tile" => {
            match provider::handle_get_tile(request.payload) {
                Ok(p) => (p, String::new()),
                Err(e) => (Vec::new(), e.to_string()),
            }
        }
        "get_geoid" => (Vec::new(), "Geoid not implemented".to_string()),
