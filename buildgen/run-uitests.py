...
def play(path: str, name: str) -> tuple[str, float]:
    ...
    # Без окна: хост рисует в текстуру и гонит кадры своим часом, окна на
    # экране нет — прогон не забирает фокус у того, кто работает за машиной.
    # Посмотреть сценарий глазами — запуск руками (docs/operations/ui-tests.md).
    env["VELDMAP_HEADLESS"] = "1"
