...
def build_all():
    """Build WASM modules and Host in DEBUG mode."""
    # 1. Build WASM Modules (in GIS workspace)
    print("\n[1/2] Building WASM Modules (DEBUG)...")
    if not os.path.exists(PLUGINS_DIR):
        os.makedirs(PLUGINS_DIR)

    for module in MODULES:
        print(f"\n--- Module: {module} ---")
        run(["cargo", "build", "--manifest-path", GIS_MANIFEST, "-p", module, "--target", WASM_TARGET])
        
        # Rust lib names use underscores instead of hyphens
        wasm_file_name = module.replace("-", "_") + ".wasm"
        source_path = os.path.join("veldgis/target", WASM_TARGET, "debug", wasm_file_name)
    ...
    print("\n[2/2] Building Native Hosts (DEBUG)...")
    run(["cargo", "build", "--manifest-path", CORE_MANIFEST, "-p", "veldmap-host-gui"])
    run(["cargo", "build", "--manifest-path", CORE_MANIFEST, "-p", "veldmap-host-cli"])
...
def main():
    if len(sys.argv) > 1:
        ...
        if cmd == "clean":
            ...
            print("Usage: python3 build-debug.py [build|clean]")
            sys.exit(1)
    else:
        build_all()
        print("\n===============================")
        print("DEBUG Build complete and successful!")
        print("================================\n")
