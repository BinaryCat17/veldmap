...
pub struct PluginUiState {
    ...
    /// Где стои́т каждая названная область прокрутки, в точках. Помнится здесь,
    /// потому что сам виджет этого не переживает: состояние iced сопоставляет
    /// по месту в дереве, а форму дерева задаёт форма раскладки — панель,
    /// бывшая корнем, становится ребёнком строки, и список прыгает в начало.
    /// Имя у области своё (`Scrollable.name`), и по нему место возвращается.
    pub offsets: HashMap<String, f32>,

    /// Незакрытый вопрос платформы «где на экране этот элемент». Снимается
    /// ответом, а ответить можно только из кадра: обход идёт по раскладке, а
    /// раскладку считает отрисовка (см. locate.rs). Экран без делегированной
    /// поверхности кадров не рисует вовсе — такой вопрос так и останется без
    /// ответа, и спрашивающий узнаёт об этом по молчанию.
