...
/// Один щелчок колеса в единицах окна: так его шлёт раннер (`WHEEL_NOTCH` в
/// `main.rs`, равенство держит `buildgen/tests/test_wire_pairs.py`), и так его
/// называет всякая оконная система со времён Windows.
...
fn aim_scrollables(plugin: &mut PluginUiState, ui: &mut UserInterface<'_, UiMessage, Theme, GpuRenderer>, renderer: &GpuRenderer) {
    ...
    for (name, aim) in asked {
        ...
        // Записанная до обхода, она снимала бы с учёта и ту, что никуда не
        // попала, — а второй раз о том же не попросят: разметка присылает
        // номер просьбы, а не смещение, и повторить его некому. Строка, к
        // которой вели, так и осталась бы за краем списка.
