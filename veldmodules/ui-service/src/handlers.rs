...
pub fn handle_set_surface(state: &mut State, req: SetSurfaceRequest) {
    let Some(surface) = req.surface else {
        veldsdk::log::warn!(target: "handlers", "set_surface для '{}' пришёл без поверхности", req.plugin_id);
        return;
    };
    veldsdk::log::info!(target: "handlers", "Поверхность '{}': текстура {} ({}x{})", req.plugin_id, surface.id, req.width, req.height);
            ...
            app_proto::ui_event::Event::Frame(f) => {
                *plugin.monitor_fps.borrow_mut() = f.monitor_fps;
                ...
                // FPS-счётчик: копим кадры и раз в 5 секунд отчитываемся.
                {
                    ...
                    fps.1 += f.dt;
                    if fps.1 >= 5.0 {
                        veldsdk::log::info!(target: "perf", "{}: {:.1} FPS в среднем за {:.1}с", plugin_id, fps.0 as f32 / fps.1, fps.1);
                        *fps = (0, 0.0);
                    }
                }
...
fn dispatch_event(event: UiEventResponse) {
    ...
    veldsdk::log::info!(target: "handlers", "UI message -> '{}/{}' (value: '{}')", event.plugin_id, event.method, event.value);
...
fn render_plugin(plugin: &PluginUiState, renderer: &mut GpuRenderer, plugin_id: &str, surface_format: i32, target_texture: u64) -> anyhow::Result<()> {
    ...
    veldsdk::log::trace!(target: "render", "commands_changed={}, is_layout_dirty={}", commands_changed, *is_layout_dirty);
