...
// клавиатуру, а сборка об этом не скажет ни слова. Тестом он не закреплён:
// тестовая цель этого крейта тянет оконный бэкенд iced (softbuffer), который
// под хост здесь не собирается, а в wrap-крейт, куда build.py ходит за
// тестами, таблица не вынесена. Проверяется он поэтому только клавиатурой:
// после обновления winit
// убедиться, что буквы приходят буквами.
