...
/// клиентами разметки. Здесь их адрес для клиента; объявлены они в самом
/// ui-service (`typography.rs`), под ними же он кладёт файлы шрифтов
/// (`state.rs`), а сюда приходят тем же файлом: одно имя — одно место, где оно
/// написано.
pub use super::typography::{FONT_ICONS, FONT_MONO, FONT_UI};
