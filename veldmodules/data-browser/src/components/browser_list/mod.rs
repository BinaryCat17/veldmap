...
pub struct BrowserItem {
    ...
    pub size: u64,
    /// Ожидаемый полный размер (Content-Length), если он был виден хоть в
    /// одном progress-событии закачки этого файла в этой сессии; 0 — не
    /// известен (например, файл никогда не качался в этой сессии, или
    /// сервер не прислал Content-Length). Только для отображения "X / Y"
    /// на недокачанном файле — см. `browser_list::view`.
    pub total_size: u64,
}
