...
/// спрашивают его трое: значок глобуса в строке (горит ли), полоса хода под
/// строкой и список слоёв. Тремя выражениями по месту они разошлись бы молча:
/// значок горел бы у снятого слоя ровно до следующей пересборки разметки.
///
/// Про контур здесь не сказано ничего: контур живёт отметкой в списке, и о ней
/// говорит коробочка (см. `table::check`).
...
impl OnGlobe {
    /// Идёт ли работа — то, ради чего под строкой рисуется полоса. Доля у
    /// сборки неизвестна: растры ещё открываются, и считать в ней нечего, а
    /// пустая дорожка говорит «началось» — это первое, что нужно увидеть
    /// нажавшему значок.
    ...
    /// Есть ли снимок на шаре хоть в каком-то виде — тем и горит значок.
