...
/// строкой, список слоёв и штриховой контур, которым место снимка помечено на
/// шаре, пока картинки нет (см. `handlers::outline::send`). Четырьмя
...
impl OnGlobe {
    ...
    pub fn said(self) -> Option<String> {
        match self {
            OnGlobe::Off => None,
            OnGlobe::Asked => Some("спрашиваем каталог…".to_string()),
