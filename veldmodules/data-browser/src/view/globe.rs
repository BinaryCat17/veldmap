...
mod tests {
    ...
    #[test]
    fn temp_widths() {
        use super::{BUTTON_PAD, HINT};
        use crate::module::components::format;
        use crate::module::theme;
        println!(
            "подсказка {:.0}, размер {:.0}, «Снять с шара» {:.0}, поля {:.0}, зазор {:.0}, поле полосы {:.0}",
            format::text_width(HINT, theme::TEXT_LABEL),
            format::text_width("1017×1334", theme::TEXT_LABEL),
            format::text_width("Снять с шара", theme::TEXT_SMALL),
            BUTTON_PAD,
            crate::module::view::BAR_SPACING,
            theme::GUTTER,
        );
