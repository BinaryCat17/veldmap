...
//! севернее её. У той же коробки остаётся 3 км расхождения вместо 27, а к
//! полюсу оно растёт — на 85-й параллели это уже 11 км. Точным его сделал бы
//! только счёт вдоль самой параллели, а не по хорде.
//!
...
///
/// Шаг крупнее, чем у рисующего (у того градус): здесь ход нужен не для
/// гладкости линии, а чтобы счёт оборотов шёл по той же кривой. Мера шага —
/// длина ребра по поверхности (`geodesy::edge_span`), поэтому у параллели он
/// растёт в долготе к полюсу, а вместе с ним и остаток расхождения (см. шапку).
fn traced(ring: &[(f64, f64)]) -> Vec<(f64, f64)> {
    const MAX_EDGE_DEG: f64 = 2.0;
