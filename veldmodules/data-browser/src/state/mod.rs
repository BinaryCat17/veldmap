...
impl State {
    ...
    /// Снять подсветку перехода — везде, кроме названного списка.
    ///
    /// Подсвечена на экране одна строка: и выбранный на шаре контур, и строка,
    /// к которой привели, отвечают на один вопрос — «что здесь главное
    /// сейчас», — а у него не бывает двух ответов. Без этого подсвеченной
    /// оставалась бы и та, к которой привели полчаса назад: переход её
    /// запоминает, а забыть её больше нечему до самого ухода из папки.
    pub fn clear_targets(&mut self, except: Option<ViewId>) {
        for view in &mut self.views {
            if Some(view.id) == except {
                continue;
            }
            if let Some(listing) = view.kind.listing_mut() {
                listing.target = None;
