...
// которые могут разойтись — ровно это и случалось на возобновлении, где
// байты уже реальные, а доля ещё нулевая.
...
/// при выводе строки (components::row), так что оба места обязаны
...
/// намерении пользователя, и показывается строкой (см. components::row).
