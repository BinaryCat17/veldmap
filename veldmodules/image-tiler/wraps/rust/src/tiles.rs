...
pub struct Meta {
    ...
    /// сколько строк.
    pub rows: Vec<Row>,
    /// Какая из величин файла показана (`Described.variable`): выбор
    /// производителя либо названная в запросе. Пусто у снимка.
...
impl Meta {
    ...
    /// производитель — первая строка его таблицы уровней, которая влезает в
    /// память (`adapters::table`), — и одна фраза здесь пересказала бы одно
    /// слагаемое из многих. К тому же строка слоя режет длинную подпись посередине
...
/// * Прочитано во весь размер — чтение дошло до конца файла, а работа нет:
///   каскад дожимает уровни после последнего окна, и байты о ней не говорят
///   ничего. Взятые за долю, они держали бы полосу полной до конца работы.
...
pub struct Passes<K> {
    ...
    /// одинаково (см. `veldsdk::reply::undelivered`). Различить может только тот,
    /// кто снимал.
    ///
...
impl<K: PartialEq> Passes<K> {
    ...
    /// снятого и у упавшего (см. `veldsdk::reply::undelivered`), а поступать с
    /// ними надо по-разному — упавший это беда, снятое нами — обычный ход
    /// приближения.
    ///
...
mod tests {
    ...
    fn байты_прохода_годны_не_у_всякого_источника() {
        ...
        // Чтение дошло до конца файла, а работа ещё идёт: впереди дожатие
        // каскада.
