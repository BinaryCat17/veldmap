...
pub struct Stored {
    ...
    pub height: u32,
    bytes: u64,
    /// Логическое время последнего события с тайлом: приезда либо обращения
    /// из кадра.
    stamp: u64,
    /// Побывал ли он хоть в одном кадре. Приезд показом не считается — см.
...
impl Store {
    ...
    pub fn accept(
        ...
        // замещается: старая текстура освобождается своим Drop. Показанность
        // при этом наследуется — ячейку рисуют, а сменился под ней только
        // носитель.
        let drawn = tiles.get(&addr).is_some_and(|old| old.drawn);
        let stored = Stored {
            _texture: texture,
            _view: view,
            bind,
    ...
    /// предок. Обращение продлевает носителю жизнь в бюджете.
    ///
    ...
    pub fn carrier(&mut self, fingerprint: &str, cell: Addr, levels: u32) -> Option<(Addr, &Stored)> {
        ...
        let step = (0..=up).find(|d| self.contains(fingerprint, (level + d, x >> d, y >> d)))?;
        let addr = (level + step, x >> step, y >> step);

        self.tick += 1;
        let stored = self.sources.get_mut(fingerprint)?.get_mut(&addr)?;
        stored.stamp = self.tick;
        stored.drawn = true;
        Some((addr, stored))
    }
    ...
    /// Вон, пока не влезем: сперва то, что никто ни разу не видел, потом
    /// давно не рисовавшееся.
    ///
    /// **Приезд — не показ.** Тайл приезжает ради ближайшего кадра, но в
    /// кадр он ещё не попал, и стоит он в очереди на вытеснение впереди
    /// любого нарисованного. Считай мы приезд обращением — а так и было бы,
    /// стой у приехавшего свежее время, — пачка приездов вытесняла бы ровно
    /// то, что сейчас на экране: у нарисованного в прошлом кадре время
    /// прошлого кадра, а у приехавшего секунду назад — нынешнее. Порядок
    /// прихода бил бы свежесть показа.
    ///
    /// Что нарисованному при этом ничего не грозит, обеспечивает не эта
    /// сортировка, а потолок аппетита ([`Store::cap_tiles`]): заказ, влезающий
    /// в свою долю бюджета, вытесняет только чужие приезды.
    ///
    /// Линейный поиск жертвы честен: бюджет в четверть гигабайта — это порядка
    /// двух сотен тайлов.
    fn evict_to_budget(&mut self) {
        while self.bytes > self.budget {
            let victim = self
                .sources
                .iter()
                .flat_map(|(key, tiles)| tiles.iter().map(move |(addr, stored)| (key, addr, stored)))
                .min_by_key(|(.., stored)| eviction_key(stored.drawn, stored.stamp))
                .map(|(key, addr, _)| (key.clone(), *addr));
...
/// Ключ очереди на вытеснение: чем меньше, тем раньше вон.
///
/// Отдельной функцией, а не выражением внутри `min_by_key`: сравнивать
/// невиданное с нарисованным можно только по одному правилу, и записанное по
/// месту оно осталось бы без проверки.
fn eviction_key(drawn: bool, stamp: u64) -> (bool, u64) {
    (drawn, stamp)
}
...
mod tests {
    ...
    /// Приезд — не показ: тайл, приехавший только что, вытесняется прежде
    /// того, что рисовали в прошлом кадре. Иначе пачка приездов выносила бы из
    /// памяти ровно то, что сейчас на экране.
    #[test]
    fn приезд_показом_не_считается() {
        assert!(eviction_key(false, 100) < eviction_key(true, 1));
        // Среди невиданного — по порядку приезда, среди нарисованного — по
        // давности показа.
        assert!(eviction_key(false, 1) < eviction_key(false, 2));
        assert!(eviction_key(true, 1) < eviction_key(true, 2));
    }
