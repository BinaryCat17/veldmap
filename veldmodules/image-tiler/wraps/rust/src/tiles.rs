...
impl Fetch {
    /// Проход, затеянный ради другого уровня: смотрят уже не на то, что он
    /// производит. Отдаёт его корреляцию — убивает её вызывающий, у него для
    /// этого свой `cancel`.
...
mod tests {
    ...
    fn во_время_прохода_не_спрашивают() {
        ...
        // Проход того же уровня — не устаревший; другого — устаревший.
        assert!(fetch.stale(2).is_none());
        assert_eq!(fetch.stale(1).as_deref(), Some("c1"));
        assert!(fetch.want(&store, "s", 1, cells(1, 0..2, 0..1)).is_some());
    }
