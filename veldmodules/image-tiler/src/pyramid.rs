...
//! другой: он про целые числа.

/// Сторона тайла в пикселях. Крупный тайл — это меньше событий, файлов и
/// bind group на ту же площадь. Одна на производителя и потребителя: этот файл
/// включён и в wrap-крейт (см. шапку), так что арифметику ячеек обе стороны
/// считают отсюда. В `Described.tile` она уезжает для того, кто файла не
/// разделяет.
