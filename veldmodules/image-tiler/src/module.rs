...
/// Порядок обхода заказа: рядами, а не столбцами.
///
/// Чанки тайлового TIFF адресуются `cy * across + cx`, то есть лежат рядами.
/// Заказ хранится множеством пар `(x, y)` — по нему проверяют принадлежность,
/// и упорядочено оно по первому полю, то есть по столбцам. Пройдя его так,
/// читатель на каждом тайле прыгает через всю ширину чанковой сетки, а
/// упреждающее чтение узнаёт проход по тому, что промах пришёлся ровно за
/// концом прошлого запроса: скачок сбрасывает разгон, и всё едет блоками по
/// одному вместо мегабайтных кусков.
///
/// Прочим рукавам производства порядок безразличен — заказ они не читают
/// вовсе, а идут по источнику своим ходом.
fn by_rows(wants: &BTreeSet<(u32, u32)>) -> Vec<(u32, u32)> {
    let mut ordered: Vec<(u32, u32)> = wants.iter().copied().collect();
...
mod tests {
    ...
    /// Заказ обходится рядами. Это не вкус: чанки тайлового TIFF лежат рядами,
    /// и столбцовый обход прыгает через всю ширину чанковой сетки на каждом
    /// тайле — упреждающее чтение такой скачок читает как случайный доступ и
    /// сбрасывает разгон, так что весь проход едет блоками по одному.
    #[test]
    fn заказ_обходится_рядами_а_не_столбцами() {
        let wants: BTreeSet<(u32, u32)> =
            [(0, 0), (0, 1), (1, 0), (1, 1), (2, 0)].into_iter().collect();

        assert_eq!(by_rows(&wants), vec![(0, 0), (1, 0), (2, 0), (0, 1), (1, 1)]);
    }

    /// Ряд обходится слева направо: чанк адресуется `cy * across + cx`, и по
    /// возрастанию `cx` смещения идут подряд.
    #[test]
    fn ряд_обходится_по_возрастанию_столбца() {
        let wants: BTreeSet<(u32, u32)> = [(7, 3), (2, 3), (5, 3)].into_iter().collect();

        assert_eq!(by_rows(&wants), vec![(2, 3), (5, 3), (7, 3)]);
    }
}
