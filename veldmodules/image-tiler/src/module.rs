...
/// величины: у NetCDF это вся плоскость, до 666 МиБ. Держать два таких
/// разом лимит памяти инстанса не переживёт, поэтому тяжёлый один и живёт
...
impl State {
    ...
    /// в лимит инстанса.
    ///
    /// Числа, которые это запрещают: осевшая плоскость величины при упоре в
    /// `netcdf::PLANE_BUDGET` — до 666 МиБ (сам потолок считан по пику двух
    /// копий, а не по размеру), а рядом `jp2::DECODE_BUDGET` — 512 МиБ. Вместе
    /// 1178 при лимите инстанса в 1024. Прочие потолки декодеров мельче и
    /// поодиночке проходят, но запаса не оставляют.
    ///
...
fn produce(state: &mut State, req: &ProduceRequest, correlation: &str) -> Result<(), String> {
    ...
    // заново, — а держит он с собой отсчёты величины, то есть до 666 МиБ
    // памяти инстанса (`netcdf::PLANE_BUDGET` меряет пик чтения из двух копий,
    // и осевшая плоскость меньше него). Остаётся он у тех, к кому
...
mod tests {
    ...
    /// прохода выделяет своё, и рядом с живой плоскостью ему не хватает: у
    /// JPEG 2000 потолок прохода — 512 МиБ, а плоскость держит до 666 при
    /// лимите инстанса в 1024.
    #[test]
