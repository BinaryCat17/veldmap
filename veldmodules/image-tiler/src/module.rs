...
fn parsed<'a>(
    ...
    if state.kept(resource_id, near).is_none() {
        // Отпускается перед всяким новым разбором, а не только перед тяжёлым.
        //
        // Держать плоскость через чужую работу нечем: потолки декодеров
        // считаны против целого лимита инстанса поодиночке — 832 МБ у
        // величины, 256 у кадра без потоковой развёртки, столько же у ряда
        // чанков, 128 у их кэша, — и любая пара из них лимит переваливает.
        // Пока эти числа не научились спрашивать друг о друге, единственный
        // держащийся инвариант — один крупный едок за раз.
        //
        // Цена известна и записана: пара «квиклук и гранула» описывается и
        // производится вперемежку, и плоскость гранулы читается дважды.
        // Дешевле дважды прочитать, чем однажды упереться в лимит: там трап,
        // а за ним пустой терминальный ответ, который заказчик читает удачей.
        state.release_heavy();
