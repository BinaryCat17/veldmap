...
//! SYNERGY величина лежит одним чанком целиком, у OLCI — тремя полосами по
//! первой оси. Оконное чтение крейта (`read_f32_rows`) режет как раз по ней,
//! так что второму оно бы и помогло, — но пирамиду каскад строит с нулевого
//! уровня и по всей ширине (см. cascade.rs), и полоса ему не заменяет
//! плоскости.
...
/// Потолок величины, тянущейся по сети. У CDSE это порядка полутора мегабайт
/// в секунду, то есть потолок здесь — «сколько ждать, пока на шаре не появится
/// ничего». Минуты довольно; спектральный куб Sentinel-5P на семьсот мегабайт
/// стои́т получаса ожидания ради ответа «это измерения, а не изображение».
///
...
/// Сколько линейной памяти инстанса вправе занять одна величина: отсчёты как
/// они лежат в файле плюс они же развёрнутые в f32. Обе копии живут
/// одновременно — развёртка читает первую и пишет вторую, — и назвать границу
/// надо до чтения: перебор кончается не отказом, а трапом.
///
/// Гигабайт лимита инстанса минус запас на всё прочее: пирамиду, кэш
/// метаданных, сам код и провисание аллокатора. Сам файл в этот счёт не
/// входит вовсе — он остаётся у хоста и приходит чанками.
const PLANE_BUDGET: u64 = 832 * 1024 * 1024;
...
///
/// Мерки две, и они РАЗНЫЕ, потому что величина сжата. `held` — во что она
/// развернётся в памяти, `wire` — сколько её байт лежит в файле, то есть что
/// придётся привезти. Расходятся они в разы: у гранулы OLCI тридцать пять
/// мегабайт отсчётов лежат в четырёх с четвертью. Мерить ожидание развёрнутым
/// объёмом значит отказывать по сети файлу, который приезжает за десять секунд.
///
...
/// которого нет.
fn affordable(held: u64, wire: u64, near: bool) -> Result<(), String> {
    let mb = |bytes: u64| bytes / (1024 * 1024);
    if held > PLANE_BUDGET {
        return Err(format!(
            "величина займёт {} МБ, а в память помещается {} МБ",
            mb(held),
            mb(PLANE_BUDGET)
        ));
    }
...
pub fn describe(resource_id: u64, len: u64, near: bool) -> Result<Info, String> {
    ...
    // разом — эту и ту, что читается сейчас, — а [`PLANE_BUDGET`] стережёт
...
fn plane(
    ...
    let held = pixels.saturating_mul(u64::from(peak_per_pixel(element)));
    ...
    affordable(held, wire, near).map_err(|why| format!("NetCDF {}×{}: {}", width, height, why))?;
...
mod tests {
    ...
    fn потолок_по_сети_лечится_диском_а_потолок_памяти_не_лечится() {
        let refusal = |held, wire, near| match affordable(held, wire, near) {
            Err(why) => why,
            Ok(()) => unreachable!("величина обязана быть отвергнута"),
        };
        let between = (WIRE_PLANE + PLANE_BUDGET) / 2;

        let wire = refusal(between, between, false);
        assert!(wire.contains("скачайте"), "отказ не назвал, чем лечится: {wire}");

        // Та же величина с диска потолком не отсекается вовсе.
        assert!(affordable(between, between, true).is_ok(), "скачанное отвергнуто терпением");
        ...
        for near in [false, true] {
            let huge = refusal(PLANE_BUDGET + 1, 1024, near);
    ...
    fn пик_чтения_больший_из_двух_путей() {
        ...
        // И то, ради чего это считается: величина у самого потолка обязана
        // укладываться в лимит инстанса, а не переваливать за него.
        let pixels = PLANE_BUDGET / u64::from(peak_per_pixel(8));
        assert!(
            pixels * u64::from(peak_per_pixel(8)) <= PLANE_BUDGET,
            "величина у потолка выходит за бюджет"
        );
    }

    /// Потолок величины живёт в памяти инстанса и обязан оставлять запас: сами
    /// отсчёты, их развёртка в f32, а рядом ещё пирамида, кэш метаданных и код.
    ///
    /// Файла в этом счёте нет вовсе, и это главное, что изменил вход по
    /// источнику: он остаётся у хоста и приходит чанками, сколько бы его ни
    /// было.
    #[test]
    fn потолок_величины_укладывается_в_инстанс() {
        const INSTANCE: u64 = 1024 * 1024 * 1024;

        assert!(PLANE_BUDGET < INSTANCE, "величина не помещается в инстанс");
        assert!(
            INSTANCE - PLANE_BUDGET >= 128 * 1024 * 1024,
            "запаса на пирамиду, кэш метаданных и сам код не осталось"
        );
        assert!(
            u64::try_from(METADATA_CACHE).is_ok_and(|cache| cache < INSTANCE - PLANE_BUDGET),
            "кэш метаданных не помещается в остаток"
        );
        assert!(WIRE_PLANE < PLANE_BUDGET, "по сети терпят дольше, чем помещается в память");
    }
}
