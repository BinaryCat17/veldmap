...
/// и смотрит на шаг вперёд: изолированная негодная линия, попавшая ровно на
/// узловую и не лежащая ни на одной стороне решётки, ждёт хода, который её не
/// убавляет и не идёт по грязи, — такого хода правило не делает. Отказ здесь
/// стои́т ровно того же, чего стоил бы он без отступа вовсе, а искать дальше
/// значит перебирать прямоугольники: их `(бюджет + 1)⁴`, и у сетки OLCI это
/// четырнадцать миллионов.
fn footing(rows: Span, columns: Span, sound: impl Fn(u32, u32) -> bool) -> Option<(Span, Span)> {
    let budget =
        (rows.1.saturating_sub(rows.0)).min(columns.1.saturating_sub(columns.0)) / (TIE_GRID - 1);
    ...
    loop {
        ...
        let tried: Vec<(usize, usize)> = (0..4)
            ...
            .collect();
        // Номер стороны стои́т в ключе последним не для порядка, а чтобы ключи
        // не совпадали вовсе: выбор обязан быть одним и тем же от запуска к
        // запуску.
        let side = tried
            .iter()
            .filter(|(left, _)| *left < here)
            .min_by_key(|(left, side)| (*left, std::cmp::Reverse(now[*side]), *side))
            .or_else(|| {
                tried.iter().min_by_key(|(left, side)| (std::cmp::Reverse(now[*side]), *left, *side))
            })
            .map(|(_, side)| *side)?;
        spent[side] += 1;
...
mod tests {
    ...
    /// рванина смоделирована через строку: на настоящей грануле она гуще, и
    /// ответ от её плотности не зависит. Проверено, что ответ тот же, что
    /// правило даёт на снятой с файла маске, — строки 1…4021 и столбцы 11…320,
    /// то есть 95,7 % растра вместо пропавшей привязки.
    #[test]
