...
/// [`peak_per_pixel`]).
///
/// **Работа рядом с осевшей плоскостью** — то, что живёт вместе с ней после
/// чтения. Слагаемых тут два, и берётся большее, потому что случаются они в
/// разное время: при описании рядом читаются решётки координат (не больше
/// [`TIES_BUDGET`]), а при проходе — полосы каскада ([`cascade::bytes`]) и
/// полоса RGBA, которую проход отдаёт каскаду ([`strip_bytes`]).
///
