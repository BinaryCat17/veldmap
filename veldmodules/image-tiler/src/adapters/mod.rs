...
/// Будет ли разбор этого источника держать при себе отсчёты величины — по
/// первым байтам, до всякого разбора.
///
/// Спрашивает это владелец memo, и вопрос у него точный: тяжёлый разбор он
/// держит один и отпускает перед новым тяжёлым. Отпуская его перед каждым, он
/// выбрасывает плоскость гранулы ради заголовков соседнего квиклука — а
/// приходят они парами вперемежку, и плоскость тогда читается заново ровно
/// между своим описанием и своим производством.
///
/// Тем же признаком, каким [`describe`] разводит форматы, и в том же месте:
/// вторая копия этого правила однажды ответила бы иначе, и разошлись бы они
/// молча — лишним чтением плоскости либо двумя плоскостями в памяти разом.
pub fn heavy_head(head: &[u8]) -> bool {
    head.starts_with(netcdf::MAGIC)
}
...
mod tests {
    ...
    /// Тяжёл разбор ровно у того, чья голова названа тяжёлой, — и вопрос этот
    /// задаётся дважды: диспетчером форматов и владельцем memo.
    ///
    /// Разойдись они, беда выйдет тихой в обе стороны. Признай владелец лёгким
    /// то, что диспетчер разберёт тяжёлым, — и в памяти инстанса окажутся две
    /// плоскости, то есть трап. Признай тяжёлым лёгкое — и плоскость гранулы
    /// будет выброшена ради заголовков соседнего квиклука и прочитана заново.
    #[test]
    fn тяжесть_разбора_спрашивается_одним_признаком() {
        assert!(heavy_head(netcdf::MAGIC), "сигнатура HDF5 — тяжёлый разбор");
        // Классический NetCDF-3 — другой формат, и плоскостей он не держит.
        assert!(!heavy_head(netcdf::CLASSIC[0]));
        assert!(!heavy_head(b"\x89PNG\r\n\x1a\n"), "PNG разбирается заголовками");
        assert!(!heavy_head(b"II*\x00"), "TIFF тоже");
        assert!(!heavy_head(&[]), "пустая голова тяжёлой не объявляется");
    }
