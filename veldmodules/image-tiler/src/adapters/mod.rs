...
pub struct Info {
    ...
    /// попытке. Ноль у всех, кроме тех, чей декодер читает кадр целиком и не
    /// умещает подробные уровни в бюджет памяти (см. `jp2::finest`).
    pub finest: u32,
...
impl Info {
    /// Растр без геопривязки — им описываются все форматы, кроме GeoTIFF:
    /// места под опорные точки у них нет вовсе.
    pub fn plain(width: u32, height: u32, kind: Kind) -> Self {
        Self { width, height, kind, finest: 0, ties: Vec::new() }
    }

    /// Что закроет один проход по этому источнику — то, по чему потребитель
    /// решает, ступать ли к цели по всем уровням или пропустить промежуточные.
    ///
...
/// решает приёмник.
///
/// Что закроет проход по каждому из рукавов, объявлено в [`Info::reach`] —
/// правьте их вместе.
pub fn produce(
    resource_id: u64,
    len: u64,
