...
//! единица.
//!
//! Два цикла. Точечный ([`direct`]): тайл уровня собирается из чанков
//! ближайшей годной копии и ужимается точно в сетку уровня. Проход ([`pass`]):
//! ряды чанков базового образа сверху вниз уезжают полосами в каскад, и все
//! уровни строятся разом. Что уровню достаётся — решает [`Grid::pointwise`] по
//! геометрии сетки и кэшу чанков; один этот ответ читают описание
//! (`Info::windowed`, `Info::reach`) и производство (`adapters::produce`).
//!
//! Всё, что о формате, лежит за трейтом [`Chunked`]: сегодня его реализует
//! TIFF (`tiff::Chunks`), драйверу от формата нужен только чанк в RGBA.
...
/// [`Grid::pointwise`]). Перевалить за потолок она может, когда ближайшая
/// годная копия крупнее уровня по стороне больше, чем во столько раз, сколько
/// тайлов укладывается в сторону потолка: честнее отказать, чем молча
/// прочитать пол-файла.
///
/// Годная — по обеим сторонам (см. [`Grid::source_for`]), поэтому дорог сюда
/// два: пропуск четырёх уровней в цепочке копий и вытянутый растр, у которого
/// короткая сторона уровня в один-два пикселя, — там отбрасывается всякая
/// копия, и уровень читается из базового образа.
pub(super) const REGION_CAP: u64 = 4096 * 4096;
...
pub(super) const BAND_CAP: u64 = 256 * 1024 * 1024;
...
pub(super) const CHUNK_CACHE_BYTES: usize = 128 * 1024 * 1024;
...
impl Grid {
    ...
    /// `base` — размеры базового образа.
    ///
    /// Меряется не площадь области, а СКОЛЬКО ПРИДЁТСЯ ДЕРЖАТЬ, чтобы отдать
    /// один тайл, и разница здесь несущая. У полосного источника чанк — это
    /// строки во всю ширину растра: тайлу нужны все полосы, накрывающие его
    /// строки, и узость тайла не экономит ничего. Влезут они в кэш чанков —
    /// соседний тайл того же ряда возьмёт их готовыми; не влезут — перечитает
    /// заново, и ряд из семи тайлов обойдётся в семь чтений файла вместо
    /// одного. Площадь этого не видит вовсе: она засчитывает ширину, которой
    /// у полосного файла экономии нет.
    ///
    /// Область при этом считается по ХУДШЕМУ тайлу сетки, а не по идеальному:
    /// границы разводятся наружу, и настоящая область бывает на пиксель шире
    /// (см. [`region_of`]). Сказать «окно» там, где [`direct`] откажет по
    ...
    pub fn pointwise(&self, base: (u32, u32), level: u32) -> bool {
        ...
        // Полосный файл этим и отличается — чанк у него во всю ширину копии,
        // и сузить его нечем, — но правило одно на оба рукава, потому что
        // тайловый с крупным тайлом ведёт себя ровно так же.
...
pub fn pass<S: Chunked>(source: &mut S, grid: &Grid, base: (u32, u32), emit: Emit) -> Result<(), String> {
    ...
    if u64::from(width) * u64::from(ch) * 4 > BAND_CAP {
        return Err(format!("ряд чанков {}×{} не влезает в бюджет прохода", width, ch));
    }
