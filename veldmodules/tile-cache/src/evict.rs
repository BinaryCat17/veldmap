...
//! закреплённая тестом; удаление — файл за файлом, fire-and-forget. Пустой
//! каталог после жертвы остаётся: fs не умеет удалять каталоги, и учить его
//! ради косметики не стоит — имя переиспользует следующая сборка того же
//! источника.
//!
