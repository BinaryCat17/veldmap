...
/// Раз за сессию на источник — свежесть меряется днями, а не миллисекундами.
...
pub fn write_file(state: &mut State, path: String, data: &[u8]) {
    ...
    let correlation = state.pending_writes.begin(PendingWrite { region: region.id(), path: path.clone() });
