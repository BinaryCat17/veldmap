...
pub enum Binding {
    ...
    /// сказано всё.
    Projected,
    /// Решётка опорных точек из растра. Старше проекции, и не по знатности
    /// источника: решётка описывает раскладку **нелинейную** — снимок радара
    /// лежит в геометрии съёмки, — а шестёрка чисел линейна по определению.
    /// Подменить решётку проекцией значит соврать ровно тем, ради отказа от
    /// чего проекционная решётка и отвергается в тайлере.
    Lattice,
}
...
impl Overlay {
    ...
    pub fn trouble(&self) -> String {
        let said: Vec<&str> =
            self.rasters.iter().filter_map(|raster| raster.trouble.as_deref()).collect();
        said.join("; ")
    }
