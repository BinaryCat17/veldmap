...
impl Overlay {
    ...
    fn on_screen(&self, meta: &Meta, level: u32, x: u32, y: u32, look: &Look) -> bool {
        ...
        for (px, py) in points {
            ...
            //
            // Сегодня сюда не попадают: видимость меряется по точке на общем
            // выносе, а камера не спускается ниже `camera::HEIGHT_RANGE_M.0` —
            // между ними без малого десять километров. Ответ здесь всё равно
            // свой, а не через `faces`: связь этих величин нигде не объявлена,
            // и первая же правка пола камеры сделала бы ветку живой — а
            // `faces || faces_eye(point, eye)` тождественно `faces`, то есть
            // отвечало бы «не видно» по одной уже пройденной точке.
            if clip[3] <= 0.0 {
                return true;
            }
            for axis in 0..2 {
