...
impl Outlines {
    ...
    ///
    /// Потолка густоты у веера, в отличие от варп-сетки наложений, нет: кольца
    /// съёмок мелки, и делить их до предела дёшево. Кольцу, дотянувшемуся до
    /// [`FILL_REACH`], веер обошёлся бы сотнями тысяч треугольников — появись
    /// такое, лечится это тем же приёмом, что у наложений: потолок долей, а
    /// остаток провала — подъёмом.
    ///
