...
///
/// Короткую сторону через антимеридиан дуга выбирает сама. У параллели выбирать
/// не приходится и нельзя: долгота идёт ровно так, как её записал каталог, —
/// развёрнутая к ближней ветви, полная окружность свернулась бы в ноль.
///
...
fn edge(ring: &mut Vec<Vertex>, from: (f64, f64), to: (f64, f64)) {
    ...
    let span = match along {
        ...
        Along::Parallel => (to.1 - from.1).abs() * from.0.to_radians().cos(),
    ...
    for step in 0..steps {
        ...
        let (lat_deg, lon_deg) = match along {
            Along::Parallel => (from.0, from.1 + (to.1 - from.1) * t),
