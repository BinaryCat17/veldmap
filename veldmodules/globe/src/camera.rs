...
///
/// Ниже спускаться мешает не сетка, а точность самих координат: позиция уезжает
/// в шейдер долями большой полуоси в `f32`, и шаг такого числа у единицы — около
/// 0,8 метра. На десяти километрах это шестая часть пикселя, на полутора — уже
/// целый, и ниже вершины начали бы дрожать и слипаться. Опустить пол можно
/// только вместе с переходом на координаты, отсчитанные от чего-то ближе центра
/// Земли; заодно придётся уменьшить и `mesh::SURFACE_LIFT_M` — вынос осмыслен,
/// пока он мал против высоты, с которой смотрят.
...
mod tests {
    ...
    fn a_probe_names_the_place_that_is_drawn_there() {
        ...
        for height_m in [10_000.0, 155_000.0, 1_000_000.0] {
