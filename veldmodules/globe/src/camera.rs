...
/// до него ([`Camera::visible_deg`]). Точность координат полу тоже больше не
/// указ — она идёт от расстояния до глаза и на восьмистах метрах составляет
/// доли микрона (`geodesy::parts`).
///
/// Восемьсот метров — это треть метра на пиксель: подробнее любого снимка,
...
impl Camera {
    ...
    fn visible_deg(&self) -> f64 {
        ...
        // его. Разница — множитель `H/(H−L)`, и мала она только пока камера
        // высоко: на десяти километрах это восемь десятых процента, а на
        // восьмистах метрах одиннадцать (см. `mesh::drawn_radius`).
    ...
    pub fn view_projection(&self, aspect: f32) -> Mat4 {
        // Половина расстояния до нарисованного, а не до эллипсоида: у самого
        // пола между ними вся разница, и мерка до эллипсоида поставила бы
        // ближнюю плоскость дальше поверхности, срезав её.
...
mod tests {
    ...
    /// дала бы тут одиннадцать процентов лишку — на нынешнем полу это уже
    /// заметно, а прежний, вдесятеро выше, прятал ошибку под процент.
    #[test]
