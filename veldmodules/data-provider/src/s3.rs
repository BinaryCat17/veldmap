...
///
/// Переход между двумя видами — только здесь и в [`identifier`]. Раньше его
/// делали по месту в каждом обработчике, и открытие по сети префикс срезать
/// забывало: получался `/eodata/eodata/…`, то есть 404 на любой файл при
/// исправном скачивании того же самого.
