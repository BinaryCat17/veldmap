...
///
/// Этой же меркой спрашиваются у каталога соседи всякого продукта с дататейком
/// (`catalogue::siblings`): правило «по слайсу» времени не ограничивает вовсе,
/// своей границы у запроса нет, и берётся она отсюда. Спросить у́же, чем
/// сводишь, — значит не получить в ответе того, кого собирался приложить.
pub(crate) const BESIDE_A_SLICE_S: i64 = 10;
