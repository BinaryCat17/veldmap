...
pub fn parse(body: &[u8]) -> anyhow::Result<Vec<(Facts, DataProduct)>> {
    ...
    let parsed: Vec<(Facts, DataProduct)> = response
        ...
        .map(|raw| {
            ...
            product.unviewable = super::imagery::unviewable(
                &product.identifier,
                product.folder,
                facts.level,
                &facts.kind,
...
fn rings(geometry: Geometry) -> Vec<Ring> {
    ...
    lines
        ...
        .filter_map(|line| {
            ...
            // неразобранной геометрии — но только если оно у продукта было
            // одно: счёт там идёт по продуктам без единого кольца (см.
            // [`parse`]), и у снимка, разрезанного швом надвое, потеря половины
            // проходит молча.
...
fn place(vertex: &Vertex) -> Option<GeoPoint> {
    match vertex.as_slice() {
        // Вершина, которой на Земле нет, — не вершина. Числа сюда приходят из
        // ответа каталога как есть, и непроверенные они дорого стоят не
        // точностью, а временем: контур ведут по градусу за шаг, и ребро
        // длиной в миллион градусов рисуется миллиардом шагов.
        [lon, lat, ..] if (-90.0..=90.0).contains(lat) && (-360.0..=360.0).contains(lon) => {
...
mod tests {
    ...
    /// Вершина, которой на Земле нет, уносит кольцо целиком.
    ///
    /// Числа приходят из ответа каталога как есть, и стоят они не точности, а
    /// времени: контур ведут по градусу за шаг, и ребро в миллион градусов
    /// рисуется миллиардом шагов. Кольцо уходит целиком по тому же правилу, что
    /// и у слишком короткой вершины: фигура без одного угла — другая фигура.
    #[test]
    fn a_vertex_that_is_not_a_place_takes_the_ring_with_it() {
        let ring = |lat: f64| Geometry {
            kind: "Polygon".to_string(),
            coordinates: serde_json::json!([[[0.0, 0.0], [1.0, 0.0], [1.0, lat], [0.0, 0.0]]]),
        };
        assert_eq!(rings(ring(1.0))[0].points.len(), 3, "обычное кольцо проходит");
        assert!(rings(ring(1.0e9)).is_empty(), "широты такой нет");
        assert!(rings(ring(90.5)).is_empty(), "и такой нет — шар кончается на девяноста");

        let east = |lon: f64| Geometry {
            kind: "Polygon".to_string(),
            coordinates: serde_json::json!([[[0.0, 0.0], [lon, 0.0], [1.0, 1.0], [0.0, 0.0]]]),
        };
        assert_eq!(rings(east(359.0))[0].points.len(), 3, "долгота бывает и как 0…360");
        assert!(rings(east(1.0e9)).is_empty());
    }
