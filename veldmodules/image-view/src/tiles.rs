//! Текстуры тайлов, живущие в видеопамяти, с вытеснением по бюджету.
//!
//! Хранилище одно на модуль, а не на канву: две вкладки с одним снимком —
//! это один набор тайлов, и бюджет видеопамяти тоже один на всех. Ключ —
//! отпечаток источника и адрес в пирамиде: тот же, что у дискового кэша,
//! потому что это тот же самый тайл ярусом выше.
//!
...
impl TileStore {
    ...
    pub fn insert(
        ...
        let bind = device.tile_bind_group(&view).map_err(|e| e.to_string())?;
