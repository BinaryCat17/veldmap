...
| variable | величина | one measured quantity of a NetCDF file: a dataset whose plane of samples the tiler can show; a file holds many, the tiler shows one, chosen by CF rules and named to the viewer (`Described.variable`), and lists the others it could have shown (`Described.variables`) |
