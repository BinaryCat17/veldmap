...
| variable | величина | one measured quantity of a NetCDF file: a dataset whose plane of samples the tiler can show; a file holds many, the tiler shows one, chosen by CF rules and named to the viewer (`Described.variable`), and lists the others it could show (`Described.variables`); the preview tab or the layer row may name one of them back (`DescribeRequest.variable`, `OverlayRaster.variable`) |
