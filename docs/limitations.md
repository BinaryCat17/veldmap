...
- **The variable of a NetCDF file is chosen per view, and only among the
  ...
  empty in this granule: chosen, such a one is refused in words — in place of
  the frame on the canvas, as the layer row's complaint on the globe — and the
  list stays to pick another. A quantity the sieve drops — a flag, a
  coordinate, an ancillary precision — cannot be chosen at all.
