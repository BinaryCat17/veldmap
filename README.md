...
`veld_task_alive`. Заказчикская сторона — `veldsdk::TaskGuard`: учёт
«заведена ли» и обряды begin/end/cancel (публикацию выполняет модуль своими
стабами); `on_end` уходит ровно один раз, отменить можно только заведённую.
