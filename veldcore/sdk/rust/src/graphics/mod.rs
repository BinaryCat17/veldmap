...
///
/// Здесь потому, что правило обряда молчаливо и стоит ресурса: владелец до
/// заливки — иначе сорвавшаяся заливка оставит голый id висеть на хосте до
/// конца процесса. `COPY_DST` ниже объявлен, но заливку не решает: хост
/// добавляет его всякой текстуре, кроме буфера глубины (см. `memory.rs`).
///
