...
impl ApplicationHandler for App<'_> {
    ...
    /// и в сеть ему теперь нельзя, а начатый поход оно бросает по объявлению
    /// (см. `veldmap_host_core::Shutdown`).
...
fn main() -> anyhow::Result<()> {
    ...
    // Гашение объявляется и здесь, каким бы ни был конец цикла: `exiting` —
    // самый ранний момент, но winit зовёт его только на своём пути к выходу, а
    // не когда цикл вернул ошибку; повторное объявление ничего не стоит. Затем
    // брошенным походам в сеть даётся вернуться: рантайм разбирает драйвер
    // таймеров раньше, чем дожидается blocking-потоков, и поход, вытесненный
    // посреди опроса своего запроса, докончил бы опрос уже после разборки — на
    // её таймере (см. `Shutdown::settled`).
    veldmap_host_core::begin_shutdown();
    if !veldmap_host_core::shutdown().settled(std::time::Duration::from_secs(2)) {
        log::warn!(target: "render", "Походы в сеть не вернулись за две секунды после объявления гашения — рантайм разбирается при них");
    }
    ran?;
