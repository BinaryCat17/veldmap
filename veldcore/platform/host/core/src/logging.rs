...
//!
//! Префикс приписывается здесь, а не на местах: автор пишет только подсистему
//! (`target: "network"`), и промахнуться мимо префикса нельзя — на уровне
//! вызова его не существует. Написанный руками `target: "host"` вместо
//! `veldmap::host::network` тихо вывел бы network за пределы фильтра, и
//! отличить «не логировали» от «отфильтровали» было бы нечем. Симметрично
//! гостевой стороне — там имя плагина приписывает хост (см. abi.rs).
//!
...
//! trace.log на случай разбора. Оба — обычные env-фильтры: эвристика «важное»
//! внутри форматтера не даёт поднять уровень одной подсистеме, потому что
//! запись проходит фильтр и теряется уже после него.
