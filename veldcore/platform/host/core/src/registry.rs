...
/// Instance id, под которым исполняются вызовы от удалённых пиров.
/// Непривилегированный (не 0) и вне диапазона локальных плагинов (те стартуют
/// со 100) — удалённый вызов видит только ресурсы, явно выданные этому id
/// (по умолчанию — никакие). Идентичность пира ему не доверяется.
pub const REMOTE_INSTANCE_ID: u32 = 1;
