...
impl Dispatcher {
    ...
    pub fn publish_from(&self, topic: &str, payload: Vec<u8>, publisher: u32, correlation: &str, target: &str) {
        ...
        // Учёт операции заводится один на публикацию, а не на доставку: у
        // отменяемого топика запись в реестре задач одна, и убийство снимает
        // одного исполнителя. Держится это на том, что подписчик у такого
        // топика ровно один — отменяемым объявляется вход сервиса, а
        // подписаться на чужой вход схема не даёт. Проверяем вслух: разойдись
        // это однажды, отмена молча убивала бы одного из двоих, а заказчик
        // получал бы два конца одной операции.
        if accounted && recipients.len() > 1 {
            log::error!(target: "dispatcher",
                "[DISPATCHER] У отменяемого '{}' подписчиков {} — учёт операции всё равно один",
