...
use veldmap_rust_rpc::ui::{DrawFrame, UiDisplayCommand, ui_display_command, UiEvent, ui_event};
...
use iced_core::{Size, Theme, mouse, Color, Font, Pixels, Rectangle};
...
pub fn handle_rpc(input: Vec<u8>) -> FnResult<Vec<u8>> {
    ...
    match request.method.as_str() {
        ...
        "handle_ui_event" => {
            let event_proto = UiEvent::decode(&request.payload[..]).unwrap();
            let mut gui_lock = GUI.lock().unwrap();
            if let Some(gui) = gui_lock.as_mut() {
                if let Some(ev) = event_proto.event {
                    match ev {
                        ui_event::Event::Resize(r) => { *CANVAS_SIZE.lock().unwrap() = (r.width, r.height); }
                        _ => {}
                    }
                }
            }
